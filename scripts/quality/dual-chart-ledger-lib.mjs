...
export const DUAL_CHART_LEDGER_PATH = join(ROOT, 'docs/quality/dual-chart-ledger.json');

const ID_PATTERN = /^MF-DUAL-CHART-\d{2}$/;

/** @param {string} [path] */
export function loadDualChartLedger(path = DUAL_CHART_LEDGER_PATH) {
  const raw = readFileSync(path, 'utf8');
  return JSON.parse(raw);
}

/** @param {ReturnType<typeof loadDualChartLedger>} ledger */
export function validateDualChartLedger(ledger) {
  const errors = [];
  const warnings = [];
  ...
  for (const item of items) {
    ...
    if (!item.gate?.startsWith('quality:dual-chart-')) {
      errors.push(`${item.id}: gate inválido ${item.gate}`);
    }
  }
  ...
  if (allDone) {
    if (ready.length > 0) {
      errors.push('Programa completo: no debe haber fases READY');
    }
...
/** @param {ReturnType<typeof loadDualChartLedger>} ledger */
export function findNextDualChartPhase(ledger) {
  const validation = validateDualChartLedger(ledger);
  if (!validation.ok) return { ...validation, next: null };
  return { ...validation, next: validation.ready ?? validation.inProgress };
}

/** @param {ReturnType<typeof loadDualChartLedger>} ledger @param {number} throughPhase */
export function phasesThrough(ledger, throughPhase) {
  return (ledger.phases ?? []).filter((p) => p.phase <= throughPhase);
}
