...
import { appendAuditEvent, listAuditEvents } from './auditLog.js';
import {
  createAuthenticate,
...
export async function registerAuthRoutes(app: FastifyInstance, config: AppConfig) {
  ...
  app.get(
    ...
    async (_request, reply) => {
      const loginEvents = listAuditEvents(200).filter((e) => e.type.startsWith('auth.'));
      return reply.send({
        events: loginEvents.map((e) => auditEventSchema.parse(e)),
      });
    },
  );
}
