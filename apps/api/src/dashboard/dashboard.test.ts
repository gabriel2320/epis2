...
describe('dashboard API', () => {
  ...
  it('GET /api/dashboard/or requiere sesión', async () => {
    const app = await buildApp(testApiConfig);
    const res = await app.inject({ method: 'GET', url: '/api/dashboard/or' });
    expect(res.statusCode).toBe(401);
  });

  it('GET /api/dashboard/aps requiere sesión', async () => {
    const app = await buildApp(testApiConfig);
    const res = await app.inject({ method: 'GET', url: '/api/dashboard/aps' });
    expect(res.statusCode).toBe(401);
  });

  it('GET /api/dashboard/specialty requiere sesión', async () => {
    const app = await buildApp(testApiConfig);
    const res = await app.inject({ method: 'GET', url: '/api/dashboard/specialty' });
    expect(res.statusCode).toBe(401);
  });
