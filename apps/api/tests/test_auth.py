...
from epis2_api.services.auth import (
    AuthError,
...
def test_login_and_me_return_local_user(client: TestClient) -> None:
    login_response = client.post(
        "/api/v1/auth/login",
        json={"email": "medico@epis2.local", "password": "medico"},
    )
    ...
    assert me_response.json()["email"] == "medico@epis2.local"
...
def test_login_rejects_invalid_password(client: TestClient) -> None:
    response = client.post(
        "/api/v1/auth/login",
        json={"email": "medico@epis2.local", "password": "wrong"},
    )
...
def test_token_expiration_is_enforced() -> None:
    ...
    user = authenticate_local_user(settings, "medico@epis2.local", "medico")
