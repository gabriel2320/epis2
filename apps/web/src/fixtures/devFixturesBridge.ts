...
/** Dev-only — production builds never load @epis2/test-fixtures (RH-06). */
...
function fx(): FixturesModule | null {
  return import.meta.env.PROD ? null : fixtures;
}

function toDemoCaseRef(demo: NonNullable<ReturnType<FixturesModule['getDemoCaseByPatientId']>>): DemoClinicalCaseRef {
  return {
...
export function hasDemoTraditionalSectionContent(
  ...
  return fx()?.hasDemoTraditionalSectionContent(demoCaseCode, sectionId) ?? true;
}
