...
export function validateRut(input: string): RutValidationResult {
  ...
  if (cleaned.length < 2) {
    return { valid: false, normalized: null, body: null, verifier: null, error: "RUT demasiado corto" };
  }
  ...
  if (body.length < 7 || body.length > 8) {
    return { valid: false, normalized: null, body: null, verifier: null, error: "RUT debe tener 7-8 dígitos" };
  }
